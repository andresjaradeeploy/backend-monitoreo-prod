...
public class CorreoAlertaImp  implements ICorreoAlertaDao{
	...
	private static final String  SQL_SELECTBYCUENTA= "select id_correo,cuenta_correo "
	+"from correo_alerta "
	+"where cuenta_fb = ? ";
	...
	public List<CorreoAlerta> correosByCuenta(String cuenta) {
		...
		public static void main(String[] args) {
			CorreoAlertaImp alertaImp= new CorreoAlertaImp();
			List<CorreoAlerta> dirCorreos = new ArrayList<>();
			dirCorreos=alertaImp.correosByCuenta("act_1434207073441086");
			for (CorreoAlerta correoAlerta : dirCorreos) {
				System.out.println(correoAlerta.getCuentaCorreo());
			}
		}
}
